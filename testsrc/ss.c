...
static struct httphandler_client * acceptclientconnection(int svrsock, SSL_CTX * ctx) {
	struct socketctx_client * client;
	struct httphandler_client * httpclient;
	int res;

	client = socketctx_openssl_accept_prepare(svrsock, ctx);
	res = socketctx_openssl_accept(client);
	if (client == NULL) {
		debug("client was null");
		return NULL;
	}

	httpclient = httphandler_new();
	httpclient->read = (int (*)(void * ctx, void * buf, size_t maxlen))&socketctx_openssl_read;
	httpclient->readptr = client;
...
next_activity:
	...
	if (client != NULL) {
		int clisock = SOCKETCTX_CLIENT_GETSOCKET(client->readptr);
		if (client->state == HTTPHANDLER_WAITINGTOREAD) {
			FD_SET(clisock, &socks_read);
			maxfd = MAX(clisock, maxfd);
		}
		if (client->state == HTTPHANDLER_WAITINGTOWRITE) {
